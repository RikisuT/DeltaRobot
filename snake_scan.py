"""Generate a snake scan trajectory from workspace point samples."""
...
def main():
    ...
    z_plane = -180
    tolerance = 1.0
    plane_points = workspace_df[np.isclose(workspace_df["Z"], z_plane, atol=tolerance)]
    ...
    scan_y_values = np.arange(np.max(hull_y_padded), np.min(hull_y_padded), -y_step)
    ...
    for y in scan_y_values:
        ...
        x_vals_at_y = [
            x
            for x in np.linspace(np.min(hull_x_padded), np.max(hull_x_padded), num_points)
    ...
    plt.scatter(x_plane_points, y_plane_points, color="red", s=2, label="Workspace Points")
    plt.plot(hull_x, hull_y, "g-", linewidth=2, label="Original Boundary")
    plt.plot(hull_x_padded, hull_y_padded, "purple", linestyle="--", linewidth=2, label="Padded Hull")
